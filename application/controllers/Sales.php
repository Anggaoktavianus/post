...
class Sales extends CI_Controller
{
    ...
    function simpan_penjualan_grosir()
    {
        ...
        if (!empty($total) && !empty($jml_uang)) {
            if ($jml_uang < $total) {
                ...
                $order_proses = $this->m_sales->simpan_penjualan_grosir($nofak, $total, $jml_uang, $kembalian, $pembeli);
                if ($order_proses) {
                    $this->cart->destroy();
                    //$this->session->unset_userdata('nofak');
                    $this->session->unset_userdata('tglfak');
                    $this->session->unset_userdata('suplier');
                    redirect('sales/cetak_struk');
                } else {
                    redirect('sales');
                }
            }
    ...
    {
        $x['data'] = $this->m_sales->cetak_struk();
        $this->load->view('transaksi/v_struk_grosir', $x);
        //$this->session->unset_userdata('nofak');
        redirect('sales');
    }
}
